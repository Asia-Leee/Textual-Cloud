...
class UCFDataset(data.Dataset):
    ...
    def get_current_seen_and_unseen_class_names_and_ids(self):  
        if self.dataset_split == "train":
            ...
            self.current_seen_class_names = np.concatenate(
                (self.train_train_class_names, self.val_unseen_class_names))
            self.current_unseen_class_names = np.array([])
        ...
        self.current_seen_class_ids = np.asarray(
            [self.all_class_name_map_idx[name] for name in self.current_seen_class_names])
        self.current_unseen_class_ids = np.asarray(
            [self.all_class_name_map_idx[name] for name in self.current_unseen_class_names])
        self.current_seen_unseen_class_ids = np.sort(
            np.concatenate((self.current_seen_class_ids, self.current_unseen_class_ids)))
    ...
    def check_exist(self):
        self.train_file_path = Path(
            "/Dataset/UCF/_features_processed/cls_features_non_averaged/trainingcls_split.pkl")
        self.trainval_file_path = Path(
            "/Dataset/UCF/_features_processed/cls_features_non_averaged/train_valcls_split.pkl")
        self.val_file_path = Path(
            "/Dataset/UCF/_features_processed/cls_features_non_averaged/valcls_split.pkl")
        self.test_file_path = Path(
            "/Dataset/UCF/_features_processed/cls_features_non_averaged/testcls_split.pkl")
    ...
    def textual_cloud(self):
        textual_cloud_embedding_path = \
            "/Dataset/ucf_descriptions_llama_(500)_top50.pt"
        ori_textual_cloud_embeddings = torch.load(textual_cloud_embedding_path, map_location='cpu')

        df=pd.read_csv("/Dataset/UCF/class-split/ucf_w2v_class_names.csv")
        ...
        # self.all_textual_cloud_embeddings_dict = {k: ori_textual_cloud_embeddings[k] for k in self.all_class_names}
        # self.all_textual_cloud_embeddings_list = [ori_textual_cloud_embeddings[k] for k in self.all_class_names]
