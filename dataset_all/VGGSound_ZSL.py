...
class VGGSoundDataset(data.Dataset):
    def __init__(self, args, dataset_split, ):
        ...
        self.zero_shot_split = args.zero_shot_split  # cls_split
    ...
    def check_exist(self):
        self.train_file_path = Path(
            "/Dataset/VGGSound/_features_processed/cls_features_non_averaged/trainingcls_split.pkl")
        self.trainval_file_path = Path(
            "/Dataset/VGGSound/_features_processed/cls_features_non_averaged/train_valcls_split.pkl")
        self.val_file_path = Path(
            "/Dataset/VGGSound/_features_processed/cls_features_non_averaged/valcls_split.pkl")
        self.test_file_path = Path(
            "/Dataset/VGGSound/_features_processed/cls_features_non_averaged/testcls_split.pkl")
    ...
    def textual_cloud(self):
        textual_cloud_embedding_path = \
            "/Dataset/vggsound_descriptions_llama_(500)_top50.pt"
        ori_textual_cloud_embeddings = torch.load(textual_cloud_embedding_path, map_location='cpu')

        df=pd.read_csv("/Dataset/VGGSound/class-split/vggsound_w2v_class_names.csv")
